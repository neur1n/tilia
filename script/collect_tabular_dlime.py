...
if __name__ == '__main__':
    ...
    for d, ds in enumerate(dataset):
        ...
        for idx in tqdm.tqdm(range(args.sample), desc="Sample"):
            subset = X_clustered[X_clustered[:, -1] == ac.labels_[ac.labels_[index_nn[idx]]]]
            subset = subset[:, :-1]
            ...
            for j, seed in enumerate(tqdm.tqdm(config.SEED, desc="Seed", leave=False)):
                ...
                if True:
                    for l in range(len(ds.label)):
                        explanation: list[tuple] = exp_inst.as_list_zero(label=l)
            ...
            for l in range(len(ds.label)):
                ...
                for feature, column in df[l].items():
                    ...
                    ent[feature] = sp.stats.entropy(cnt, base=2)
                    """ aad using mode """
