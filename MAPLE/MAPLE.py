...
class MAPLE:
    ...
    def __init__(self, X_train, MR_train, X_val, MR_val, fe_type = "rf", n_estimators = 200, max_features = 0.5, min_samples_leaf = 10, regularization = 0.001, random_state = None):
        ...
        for retain in range(1, num_features + 1):
            ...
            for i in range(num_val):
                ...
                # Local linear model
                lr_model = Ridge(alpha=regularization, random_state=self.random_state)
    ...
    def explain(self, x):
        ...
        # Local linear model
        lr_model = Ridge(alpha = self.regularization, random_state=self.random_state)
        ...
        out["coefs"] = coefs
        out["pred"] = prediction
        out["score"] = lr_model.score(self.X, self.MR_train, weights)
